...
class acf_options_page_plugin
{
	...
	*/
	...
	function __construct($options)
	{
		...
		/*// vars
		$this->settings = array(
			'title' => __('Wristband Manager','acf'), // title / menu name ('Site Options')
			'capability' => 'edit_posts', // capability to view options page
			'pages' => array('Sizes','Color Styles', 'Color Style Price List', 'Colors', 'Messages', 'Fonts', 'Logo', 'Additional Options', 'Additional Option Price List', 'Customization Location', 'Dates For Customization', 'Customization Options', 'Production Price List', 'Shipping Price List'), // an array of sub pages ('Header, Footer, Home, etc')
		);
		*/
		...
		if (count($this->settings) != 0) {
			...
			foreach ($this->settings as $i => $settings) {
				...
				if (isset($GLOBALS['acf_options_pages']) && !empty($GLOBALS['acf_options_pages'])) {
					$this->settings[$i]['pages'] = array_merge($settings['pages'], $GLOBALS['acf_options_pages']);
				}
			}
		}
	}
	...
	*/
	function acf_location_rules_values_options_page( $choices )
	{
		...
		foreach ($this->settings as $i => $settings) {
			if (!is_array($settings)) continue;
			$choices = array(
				$settings['slug'] => $settings['title']
			);

			$titles = $settings['pages'];
			if (!empty($titles)) {
				$choices = array();
				foreach ($titles as $title) {
					$slug = (isset($settings['slug']) && !empty($settings['slug']) ? $settings['slug'] : 'acf-options'). '-' . sanitize_title($title);
					$choices[$slug] = $title;
				}
			}
		}
	...
	*/
	...
	function admin_menu()
	{
		...
		foreach ($this->settings as $i => $settings) {
			...
			$parent_title = $settings['title'];
			$parent_menu = $settings['title'];


			// redirect to first child
			if (!empty($settings['pages'])) {
				$parent_title = $settings['pages'][0];

				$parent_slug = (isset($settings['slug']) && !empty($settings['slug']) ? $settings['slug'] : 'acf-options'). '-' . sanitize_title($parent_title);
			}

			$icon = isset($settings['icon']) && !empty($settings['icon']) ? $settings['icon'] : false;
			$menu_order = isset($settings['menu_order']) && !empty($settings['menu_order']) ? $settings['menu_order'] : false;
			// Parent
			$parent_page = add_menu_page($parent_title, $parent_menu, $settings['capability'], $parent_slug, array($this, 'html'), $icon, $menu_order);
	...
	function html()
	{
		...
		<div class="wrap no_move">
			...
			<div class="metabox-holder has-right-sidebar" id="poststuff">
				...
				<!-- Sidebar -->
				<div class="inner-normal" id="side-info-column">
